...
from .models import (CompanyInformation, YourBusinessRecommendation, YourCurrentTradelines, VudokeBusinessIntelligenceScore, BusinessReports, UpdateYourCompanyInformation,
                     Dispute, VukodeScore)
...
def index(request):
    ...
    recommendation = YourBusinessRecommendation.objects.filter(
        user=request.user)
    context = {}
    context['score'] = score
...
def BusinessCreditReports(request):
    ...
    context = {}
    context['reports'] = reports
    score = VudokeBusinessIntelligenceScore.objects.filter(
...
def UpdateCompanyInformation(request):
    context = {}
    company = UpdateYourCompanyInformation.objects.filter(
        user=request.user).first()
    context['company'] = company
